...
from helical.models.hyena_dna_generator.model.modeling_hyena import HyenaDNAForCausalLM
from helical.models.hyena_dna_generator.model.tokenization_hyena import HyenaDNATokenizer
import torch
import random
...
def generate(
    ...
    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        with torch.no_grad():
            start_letter = random.choice(["A", "C", "G", "T"])
...
def compute_sequence_log_likelihood_hyena(model, sequences, tokenizer, device="cuda", normalize=False):
    """
...
def mutate_sequence_hyena(
    ...
    if mutation_position == 0:
        ...
        start_letters = np.random.choice(["A", "C", "G", "T"], size=batch_size).tolist()
...
if __name__ == "__main__":
    ...
    # Load the language modelling version of HyenaDNA model and tokenizer
    model = HyenaDNAForCausalLM.from_pretrained(
        "LongSafari/hyenadna-medium-160k-seqlen-hf",
    ).to("cuda")

    tokenizer = HyenaDNATokenizer(padding_side="right", model_max_length=160_000)
    # Print the vocbulary 
    ...
    log_likelihoods = compute_sequence_log_likelihood_hyena(model, mutated, tokenizer=tokenizer, device=device)
...
#['AAAAATGTTT']
# [array([0.        , 0.31936178, 0.38707545, 0.38104078, 0.40297326,
#        0.25333503, 0.22596394, 0.26847702, 0.30814734, 0.37773448],
#       dtype=float32)]